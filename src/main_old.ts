...
function computeState(state: GameState, gameMessage: GameMessage): GameState {
  ...
  switch (gameMessage.tag) {
    ...
    case SET_NICK:
      ...
      const playerJoined: Player = newPlayer(gameMessage.user, gameMessage.name);
      return newGameState(addPlayer(playerJoined, statePlayers));
...
function enterRoom() {
    ...
    client.init('ws://localhost:8080').then(() => {
      ...
      function sendPlayerJoinedEvent() {
          ...
          client.send(testRoom, lib.encode(setNickMsg));
        }
        ...
        client.recv(testRoom, (msg: Uint8Array) => {
            ...
            if (decodedMessage.tag == SET_NICK) {
              state = computeState(state, decodedMessage);
            } else {
