...
import { newPlayer, getPlayerById, updatePlayer, addPlayer } from "./player";
import { addMessageToQueue, newGameState } from "./gamestate";
import { GameState, GameMessage, Player, StateHistory } from "./types";
...
let gameState: GameState = newGameState(List(), 0, [], false);
let messageQueue: GameMessage[] = [];
let stateHistory: StateHistory = [];

// Funções
function thisPlayer(): number {
  return thisPlayerId;
}

function setThisPlayer(p: number): void {
  thisPlayerId = p;
}
...
//   return { players, tick, messages, tickSet };
// }
